...
public class NotificationDataTest {
	...
	public void test() throws JAXBException, IOException {
		...
		for (int i=0;i<2;i++) {
			flatList.putEntry(new SerializableEntryType(createEntryType("sip:entry"+i+"@example.com", "entry"+i)));
		}
		...
		String rlmi1 = "<?xml version=\"1.0\" encoding=\"UTF-8\" standalone=\"yes\"?>" +
		...
		StringReader stringReader = new StringReader(rlmi1);
		...
		String rlmi2 = "<?xml version=\"1.0\" encoding=\"UTF-8\" standalone=\"yes\"?>" +
		"<list fullState=\"true\" version=\"0\" uri=\""+notifier+"\" xmlns=\"urn:ietf:params:xml:ns:rlmi\">" +
		"<resource uri=\"sip:entry1@example.com\">"+
		"<name>entry1</name>"+
		"<instance id=\"id1\" state=\"active\" cid=\"cid1\"/>"+
		...
		stringReader = new StringReader(rlmi2);
		rlmiUnmarshalled = unmarshaller.unmarshal(stringReader);
		stringReader.close();
		stringWriter = new StringWriter();
		marshaller.marshal(rlmiUnmarshalled, stringWriter);
		rlmi2 = stringWriter.toString();
		stringWriter.close();
		...
		String multiPartString = multiPart.toString();
		
		System.out.println("### Multipart object:\n"+multiPartString);
		Assert.assertTrue("multiPart.toString() is not the expected!", mp1.equals(multiPartString) || mp2.equals(multiPartString));
