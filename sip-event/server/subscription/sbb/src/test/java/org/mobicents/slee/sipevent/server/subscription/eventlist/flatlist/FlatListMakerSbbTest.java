...
import org.mobicents.slee.sipevent.server.subscription.eventlist.SerializableEntryType;
...
public class FlatListMakerSbbTest implements FlatListMakerParentSbbLocalObject {
	...
	public void test() throws JAXBException, IOException, InterruptedException {
		...
		FlatListMakerSbbImpl flatListMaker = new FlatListMakerSbbImpl();
	...
	public void flatListMade(FlatList flatList) {
		...
		for (Iterator<SerializableEntryType> it = flatList.getEntries().values().iterator();it.hasNext();) {
			System.out.println("Entry with uri "+it.next().getPojo().getUri());
		}
	}
